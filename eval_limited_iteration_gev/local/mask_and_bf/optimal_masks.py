...
class MaskBfEstimator(nn.Module):
    def __init__(self, bins, frames, channels, mode, scaling, do_bn):
        super().__init__()
    ...
    def _mask_based_bf(self, x, s, target_ch):
        ...
        if self.mode_idx == MODE_BOTH: # startswith('both_'):
        ...
        else: #if self.mode_idx == MODE_RIGHT:
        ...
        scaling_target = self._get_scaling_target(x, s, target_ch)
        y = self._rescale(y, scaling_target)

        return y.T
    
    def _get_scaling_target(self, x, s, target_ch):
        if self.scaling_method in ('mask', 'normask', 'sqnormask', 'absmask'):
            mask = self.get_scaling_mask()
            return mask.T * x[:,target_ch:target_ch+1]
        elif self.scaling_method == 'ideal':
            return s[:,target_ch:target_ch+1]
        elif self.scaling_method == 'mdp':
            return x[:,target_ch:target_ch+1]
        else:
...
parser.add_argument('--mode',
                    help='Mode. (both_max|both_min|left_max|left_min|right_max|right_min)')
parser.add_argument('--scaling',
                    help='Scaling method. (mask|mdp|ideal|normask|sqnormask|absmask)')
...
if args.scaling not in ('mask', 'mdp', 'ideal', 'normask', 'sqnormask', 'absmask'):
...
for cur_line in tqdm(flist[:last_idx]):
    ...
    model = MaskBfEstimator(bins=bins, frames=frames, channels=channels, mode=args.mode, scaling=args.scaling, do_bn=args.do_bn)
    if args.gpu >= 0:
        model.cuda(args.gpu)
        X = X.cuda(args.gpu)
        S = S.cuda(args.gpu)

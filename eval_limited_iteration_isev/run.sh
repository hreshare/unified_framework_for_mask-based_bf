...
stage=1
#stage=3
enhancement_base=limited_iteration_isev #### or your method
...
# w <-- inv(<m1 xx^H>) SEV(<xx^H>)  left
# w <-- inv(<xx^H>) SEV(<m1 xx^H>)  right
# w <-- inv(<m1xx^H>)SEV(<m2 xx^H>) both
...
enhancement=${enhancement_base}_bg-${bg_gain}_mode-${mode}_scale-${scaling}_itr-${iteration}_bn-${do_bn}

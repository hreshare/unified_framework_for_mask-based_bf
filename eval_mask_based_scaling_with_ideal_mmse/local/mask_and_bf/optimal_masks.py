...
def plot_specgrams(X, masks, Y, S, N, fig_no=0):
    #print(X.shape, masks.shape, Y.shape, S.shape, N.shape)
    X = np.abs(to_np(X))
    Y = np.abs(to_np(Y))
    S = np.abs(to_np(S))
    N = np.abs(to_np(N))
...
class ScalingMaskEstimator(nn.Module):
    def __init__(self, bins, frames, channels, mode, masks, scaling, target_ch, do_bn):
        super().__init__()
        self.masks_num = MODE_TO_MASK_NUM[mode]
        self.w = None
        self.masks = masks
        self.bins = bins
        self.frames = frames
    ...
    def _mask_based_bf(self, x, s):
        ...
        ch = self.target_ch
        if self.w is None:
            #print('Computing BF filter', self.mode_idx, self.masks_num, self.minimize)
            if self.mode == 'ideal_mwf':
            ...
            self.w = torch.linalg.solve(wcov1, wcov2[:,:,ch:ch+1])
        y = torch.matmul(self._htp(self.w), x)
...
if args.mode == 'ideal_mwf':
    ...
    MASK_BASE_DIR = os.path.join(
        ...
        'enhan',
        'limited_iteration_inv_bg-{}_mode-{}_scale-ideal_itr-500_bn-True'.format(args.bg_gain, args.mode),
    )
...
for cur_line in tqdm(flist[:last_idx]):
    ...
    model = ScalingMaskEstimator(bins=bins, frames=frames, channels=channels, mode=args.mode, masks=masks, scaling=args.scaling, target_ch=target_ch, do_bn=args.do_bn)
    if args.gpu >= 0:
        model.cuda(args.gpu)
        X = X.cuda(args.gpu)
        S = S.cuda(args.gpu)

    if args.scaling not in ('ideal', 'mdp', 'none'):
        # Setup optimizer
